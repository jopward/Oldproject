...
def update_attendance():
    ...
    existing = db_session.execute(
        'SELECT * FROM attendance WHERE student_id = :student_id AND date = :today AND school_id = :school_id AND teacher_id = :teacher_id',
        {"student_id": student_id, "today": today, "school_id": school_id, "teacher_id": teacher_id}
    ).fetchone()

    if existing:
        db_session.execute(
            'UPDATE attendance SET status = :status WHERE student_id = :student_id AND date = :today AND school_id = :school_id AND teacher_id = :teacher_id',
            {"status": status, "student_id": student_id, "today": today, "school_id": school_id, "teacher_id": teacher_id}
        )
    else:
        db_session.execute(
            'INSERT INTO attendance (student_id, date, status, school_id, teacher_id) VALUES (:student_id, :today, :status, :school_id, :teacher_id)',
