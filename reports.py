...
def reports_page():
    ...
    students = db_session.execute('''
        ...
        LEFT JOIN student_tracking t ON s.id = t.student_id AND t.date=:today AND (:teacher_id IS NULL OR t.teacher_id=:teacher_id)
        WHERE s.school_id=:school_id
        GROUP BY s.id
    ''', {"teacher_id": teacher_id, "today": today, "school_id": school_id}).fetchall()
...
def student_detail(student_id):
    ...
    ''', {"teacher_id": teacher_id, "student_id": student_id, "school_id": school_id}).fetchone()
    ...
    ''', {"teacher_id": teacher_id, "student_id": student_id}).fetchall()
...
def get_attendance_details(student_id, status):
    ...
    rows = db_session.execute(
        "SELECT date FROM attendance WHERE student_id=:student_id AND status=:status AND school_id=:school_id AND (:teacher_id IS NULL OR teacher_id=:teacher_id)",
...
def get_tracking_details(student_id, field):
    ...
    rows = db_session.execute(
        f"SELECT date FROM student_tracking WHERE student_id=:student_id AND {field}=1 AND school_id=:school_id AND (:teacher_id IS NULL OR teacher_id=:teacher_id)",
