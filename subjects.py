...
def subjects_page():
    ...
    # جلب المواد
    subjects = db_session.execute(text("SELECT * FROM subjects")).fetchall()

    # جلب المعلمين ضمن المدرسة الحالية فقط
    teachers = db_session.execute(
        text("SELECT * FROM teachers WHERE school_id = :school_id"),
        {"school_id": school_id}
    ).fetchall()
        ...
        try:
            ...
            for tid in teacher_ids:
                teacher = db_session.execute(
                    text("SELECT id FROM teachers WHERE id = :teacher_id AND school_id = :school_id"),
                    {"teacher_id": tid, "school_id": school_id}
                ).fetchone()
                if teacher:
                    db_session.execute(
                        text("INSERT INTO subject_teachers (subject_id, teacher_id) VALUES (:subject_id, :teacher_id)"),
...
def edit_subject(subject_id):
    ...
    current_teacher_ids = [t['teacher_id'] for t in db_session.execute(
        text("SELECT teacher_id FROM subject_teachers WHERE subject_id = :subject_id"),
        {"subject_id": subject_id}
    ).fetchall()]
    ...
    if request.method == "POST":
        ...
        if subject_name and teacher_ids:
            db_session.execute(
                text("UPDATE subjects SET subject_name = :subject_name WHERE id = :subject_id"),
                {"subject_name": subject_name, "subject_id": subject_id}
            )
            db_session.execute(
                text("DELETE FROM subject_teachers WHERE subject_id = :subject_id"),
                {"subject_id": subject_id}
            )
            for tid in teacher_ids:
                teacher = db_session.execute(
...
def delete_subject(subject_id):
    db_session = SessionLocal()
    try:
        db_session.execute(text("DELETE FROM subject_teachers WHERE subject_id = :subject_id"), {"subject_id": subject_id})
        db_session.execute(text("DELETE FROM subjects WHERE id = :subject_id"), {"subject_id": subject_id})
        db_session.commit()
...
def remove_teacher_from_subject(st_id):
    db_session = SessionLocal()
    try:
        db_session.execute(text("DELETE FROM subject_teachers WHERE id = :st_id"), {"st_id": st_id})
        db_session.commit()
